...
In this scope, “presenting a LUN” means registering/reconciling host WWPNs in PowerStore or creating a PowerMax masking view, then configuring zoning across the fabrics. SCSI rescan, multipath and filesystem work inside the operating system remain host-side activities, so San Flow does not need SSH credentials for servers.
