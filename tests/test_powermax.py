    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/univmax/restapi/100/sloprovisioning/symmetrix/SYM-1/storagegroup":
            if request.method == "GET":
                nonlocal list_calls
                list_calls += 1
                return httpx.Response(200, json={"storageGroupId": [] if list_calls == 1 else ["APP_SG"]})
        ...
        if request.url.path == "/univmax/restapi/100/sloprovisioning/symmetrix/SYM-1/storagegroup/APP_SG":
            return httpx.Response(200, json={"storageGroupId": "APP_SG", "name": "APP_SG"})
        return httpx.Response(404, json={"message": "unexpected"})

    with PowerMaxClient(
        "powermax.local", "api-user", "secret", api_version="100", transport=httpx.MockTransport(handler)
    ) as client:
        client.symmetrix_id = "SYM-1"
        created = client.ensure_storage_group(
            {"name": "APP_SG", "size_gib": 100, "volume_count": 2, "srp_id": "SRP_1", "slo_id": "Diamond"}
        )
