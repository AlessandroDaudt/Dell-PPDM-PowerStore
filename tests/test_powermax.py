...
def test_powermax_creates_native_storage_group_and_reuses_existing_group():
    ...
    with PowerMaxClient(
        ...
        reused = client.ensure_storage_group({"name": "APP_SG", "size_gib": 100, "volume_count": 2})
...
def test_powermax_creates_masking_view_and_host_when_missing():
    captured = {}

    def handler(request: httpx.Request) -> httpx.Response:
        path = request.url.path
        if path.endswith("/maskingview") and request.method == "GET":
            return httpx.Response(200, json={"maskingViewId": []})
        if path.endswith("/host") and request.method == "GET":
            return httpx.Response(200, json={"hostId": []})
        if path.endswith("/maskingview") and request.method == "POST":
            captured["body"] = json.loads(request.content)
            return httpx.Response(201, json={"maskingViewId": "APP_SG_host-1"})
        return httpx.Response(404, json={"message": "unexpected"})

    with PowerMaxClient(
        "powermax.local", "api-user", "secret", transport=httpx.MockTransport(handler)
    ) as client:
        client.symmetrix_id = "SYM-1"
        result = client.ensure_masking_view(
            "APP_SG",
            "host-1",
            ["10:00:00:90:fa:12:34:56"],
            "PG_PROD",
            {"masking_view_prefix": "APP_SG"},
        )

    body = captured["body"]
    assert result["id"] == "APP_SG_host-1"
    assert body["hostOrHostGroupSelection"]["createHostParam"]["initiatorId"] == [
        "10:00:00:90:fa:12:34:56"
    ]
    assert body["portGroupSelection"]["useExistingPortGroupParam"]["portGroupId"] == "PG_PROD"
